async function load() {
    ...
    //Adding Select Menu for Containers
    const containers = await browser.contextualIdentities.query({});
    const selectMenu = document.getElementById("open-in-container-select-menu");
    //create option for each container and append to select menu
    containers.forEach(container => {
        const option = document.createElement("option");
        option.value = container.cookieStoreId;
        option.text = browser.i18n.getMessage("openInContainer", container.name);
        option.addEventListener("click", (e) => {
            e.preventDefault();
            confirmSubmit(redirectUrl, container.cookieStoreId);
        });
        selectMenu.appendChild(option);
    });
    //when enter is pressed on select menu, confirm submit
    selectMenu.addEventListener("keydown", (e) => {
        if (e.keyCode === 13) {
            e.preventDefault();
            confirmSubmit(redirectUrl, selectMenu.value);
        }
    });
}
